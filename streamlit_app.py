...
# 환경설정 & 유틸
# -----------------------------
load_dotenv()  # 로컬 개발용 .env (배포 환경에서는 st.secrets 권장)

# 배포: st.secrets 우선, 로컬: 환경변수 폴백
...
def validate_env() -> bool:
    if not YOUTUBE_API_KEY:
        st.error("YOUTUBE_API_KEY가 설정되지 않았습니다. 프로젝트 루트의 .env 파일에 키를 추가하세요.")
        with st.expander(".env 설정 예시"):
            st.code("""YOUTUBE_API_KEY=YOUR_API_KEY
REGION_CODE=KR
MAX_RESULTS=30
""", language="bash")
        st.stop()
